...
class CequenceAnalytics:
    ...
    def __init__(self, gateway_id: str, api_key: str):
        ...
        self._buffer_size = settings.analytics_buffer_size
    ...
    async def track_request(
        ...
        try:
            ...
            if (len(self._metrics_buffer) >= self._buffer_size or 
                (datetime.now(timezone.utc) - self._last_flush).seconds > settings.analytics_flush_interval):
    ...
    async def _send_metrics(
        ...
        try:
            ...
            if not self.gateway_id or not self.api_key:
                logger.debug("cequence_not_configured", skipping_metrics=len(metrics))
                return
...
async def get_cequence_analytics() -> CequenceAnalytics:
    ...
    if _cequence_analytics is None:
        gateway_id = settings.cequence_gateway_id or "demo_gateway"
        api_key = settings.cequence_api_key or "demo_key"
        _cequence_analytics = CequenceAnalytics(gateway_id, api_key)
...
async def track_agent_operation(
    ...
    """Convenience function to track agent operations"""
...
async def track_security_event(
    ...
    """Convenience function to track security events"""
    analytics = await get_cequence_analytics()
